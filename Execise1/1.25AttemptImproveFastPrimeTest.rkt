#lang sicp

; ==========================================================================

(define (square x) (* x x))

; attempt to improve fast prime test: do not mod for every product, just mod the final result.
(define (fast-expt base exp)
    (define (fast-expt-iter product base exp)
        (cond ((= exp 0) product)
              ((even? exp) (fast-expt-iter product (square base) (/ exp 2)))
              (else (fast-expt-iter (* product base) base (- exp 1)))
        )
    )
    (fast-expt-iter 1 base exp)
)


; O(LogN) probability prime test
(define (fast-prime? n times)
    (define (expmod base exp m)
        (remainder (fast-expt base exp) m)
    )
    (define (fermat-test n)
        (define (try-it a)
            (= (expmod a n n) a)
        )
        (try-it (+ 1 (random (- n 1)))) ; try number between 1 and n-1 (inclusive)
    )
    (cond ((= times 0) true)
          ((fermat-test n) (fast-prime? n (- times 1)))
          (else false)
    )
)

; use fast prime test
(define (prime? n)
    (fast-prime? n 3)
)
; ==========================================================================

; from 1.24
(define (next-odd n)
    (if (odd? n)
        (+ n 2)
        (+ n 1))
)

(define (timed-prime-test n)
    (start-prime-test n (runtime))
)

(define (start-prime-test n start-time)
    (if (prime? n)
        (report-prime n (- (runtime) start-time))
    )
)

(define (report-prime n elapsed-time)
    (newline)
    (display n)
    (display " : ")
    (display elapsed-time)
    (display " us")
)

(define (generate-primes-from n count)
    (if (> count 0) (timed-prime-test n))
    (cond ((= count 0) (display ""))
          ((prime? n) (generate-primes-from (next-odd n) (- count 1)))
          (else (generate-primes-from (next-odd n) count))
    )
)

; find smallest 3 prime number that greater than 1000/10000/100000/1000000.
(define (search-for-primes n)
    (newline)
    (display "from ")
    (display n)
    (generate-primes-from n 3)
)

(search-for-primes 1000)
(search-for-primes 10000)
(search-for-primes 100000)
(search-for-primes 1000000)
; (search-for-primes 10000000) ; too slow

; 结论：这并不是一个好的尝试，甚至可以作为自以为是的负优化的绝佳案例。
; 当整数位数过高时，乘法运算的复杂度与整数位数相关(平凡O(N^2)优化O(NlogN)，N为整数位数)
; 在每一步多一个取模操作可以非常有效地降低运行时间
